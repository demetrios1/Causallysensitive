...
prediction_function=function(df, treat='G', Outcome='B',vars, model='random forest'){
  ...
  if (model=='random forest'){
    ...
    #case 2 control
    mod2=randomForest::randomForest(covtreat[vars],as.factor(unlist(covcontrol[Outcome])) )
    print(mod2)
    #PB1G0
    pred2=predict(mod2, df[vars], type='prob')[1]
    #case 3 propensity
    mod3=randomForest::randomForest(covtreat[vars],as.factor(unlist(df[treat])) )
    print(mod3)
    #PG1
